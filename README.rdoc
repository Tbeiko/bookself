Welcome to Bookself (looking to be renamed bookselv.es).
...
Some users are generated by the administrators though `omniauth identity`. These are "famous bookshelves", such as Bill Gates, Mark Zuckerberg, etc. These are saved as `identities` before being saved as `users`.
...
A `user_book` item will be created between the `user` and `book`.

If the book has already been added in the past by another user, only a `user_book` will be created.
...
... I'm not very good at markdown, it seems.
